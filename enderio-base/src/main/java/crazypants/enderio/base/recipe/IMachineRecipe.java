...
public interface IMachineRecipe {
  ...
  /**
   * The amount total number of MJ required to craft the recipe based on the inputs. The amount returned should be for a single 'cycle' of the recipe. For
   * examples, if the inputs would allow the recipe to completed 10 times, this method should only return the amount of energy used for a single cycle.
   * 
   * @param inputs
   * @return
   */
  ...
  /**
   * Only returns true if output can be generated using these inputs. If partially complete inputs are provided (for example only one of two required inputs are
   * present) the method should return false.
   * 
   * @param inputs
   * @return
   */
