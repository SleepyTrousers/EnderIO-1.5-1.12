...
public class HandleIOMode implements IHandler<Map<EnumFacing, IoMode>> {
  ...
  @Override
  public Map<EnumFacing, IoMode> read(@Nonnull Registry registry, @Nonnull Set<StoreFor> phase, @Nonnull NBTTagCompound nbt, @Nonnull String name,
      @Nullable Map<EnumFacing, IoMode> object) throws IllegalArgumentException, IllegalAccessException, InstantiationException, NoHandlerFoundException {
    if (nbt.hasKey(name)) {
      if (object == null) {
