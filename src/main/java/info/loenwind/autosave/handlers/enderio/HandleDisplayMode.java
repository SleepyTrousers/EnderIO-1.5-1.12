...
public class HandleDisplayMode implements IHandler<Map<EnumFacing, InfoDisplayType>> {
  ...
  @Override
  public Map<EnumFacing, InfoDisplayType> read(@Nonnull Registry registry, @Nonnull Set<StoreFor> phase, @Nonnull NBTTagCompound nbt, @Nonnull String name,
      @Nullable Map<EnumFacing, InfoDisplayType> object) throws IllegalArgumentException, IllegalAccessException, InstantiationException,
      NoHandlerFoundException {
    if (nbt.hasKey(name)) {
      if (object == null) {
