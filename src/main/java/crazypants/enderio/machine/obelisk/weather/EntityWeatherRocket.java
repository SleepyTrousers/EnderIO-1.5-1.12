        ...
        for (int j = -size; j <= size; ++j) {
            for (int k = -size; k <= size; ++k) {
                for (int l = -size; l <= size; ++l) {
                    ...
                    double d7 = (double) MathHelper.sqrt_double(d4 * d4 + d5 * d5 + d6 * d6) / speed
                            + this.rand.nextGaussian() * 0.05D;

                    EntityFireworkSparkFX entityfireworksparkfx = new EntityFireworkSparkFX(
                            this.worldObj,
                            d1,
                            d2,
                            d3,
                    ...
                    entityfireworksparkfx.setColour(
                            WeatherTask.values()[getDataWatcher().getWatchableObjectInt(DATA_ID)].color.getRGB());
