...
public class TileEntitySolarPanel extends TileEntityEio implements IInternalPowerProvider, IWailaNBTProvider {
    ...
    private void collectEnergy() {
        if (canSeeSun()) {
            ...
            if (lastCollectionValue > 0) {
                network.setEnergyStored(
                        Math.min(lastCollectionValue + network.getEnergyStored(), network.getMaxEnergyStored()));
            }
        }
    }
    ...
    private int getEnergyPerTick() {
        ...
        if (meta == 0) return Config.maxPhotovoltaicOutputRF;
        if (meta == 1) return Config.maxPhotovoltaicAdvancedOutputRF;
