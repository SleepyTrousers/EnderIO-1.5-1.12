...
public class TileElectricLight extends TileEntityEio implements IInternalPowerReceiver {
    ...
    private boolean isLightNode(Vector3d offset) {
        return worldObj.getBlock(xCoord + (int) offset.x, yCoord + (int) offset.y, zCoord + (int) offset.z)
    ...
    private boolean isTranparent(Vector3d offset) {
        ...
        return worldObj.getBlockLightOpacity(xCoord + (int) offset.x, yCoord + (int) offset.y, zCoord + (int) offset.z)
                == 0;
    }
