...
public class TileAlloySmelter extends AbstractPoweredTaskEntity {
    ...
    public boolean isMachineItemValidForSlot(int slot, ItemStack itemstack) {
        ...
        // We will assume anything that is in a slot is valid, so just return whether the new input can be stacked with
        // the current one
        ...
        for (IMachineRecipe recipe : recipes) {
            if (!(recipe instanceof VanillaSmeltingRecipe)) {
                ...
                if (recipe instanceof ManyToOneMachineRecipe) {
                    ...
                    Log.warn(
                            "TileAlloySmelter.isMachineItemValidForSlot: A non alloy recipe was returned for the alloy smelter");
                    return true;
                }
            }
        }
