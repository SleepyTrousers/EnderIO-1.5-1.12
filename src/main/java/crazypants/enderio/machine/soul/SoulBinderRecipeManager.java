...
public class SoulBinderRecipeManager {
    ...
    public void addDefaultRecipes() {
        MachineRecipeRegistry.instance
                .registerRecipe(ModObject.blockSoulBinder.unlocalisedName, SoulBinderSpawnerRecipe.instance);
        MachineRecipeRegistry.instance
                .registerRecipe(ModObject.blockSoulBinder.unlocalisedName, SoulBinderReanimationRecipe.instance);
        MachineRecipeRegistry.instance
                .registerRecipe(ModObject.blockSoulBinder.unlocalisedName, SoulBinderSentientRecipe.instance);
        MachineRecipeRegistry.instance
                .registerRecipe(ModObject.blockSoulBinder.unlocalisedName, SoulBinderEnderCystalRecipe.instance);
        MachineRecipeRegistry.instance
                .registerRecipe(ModObject.blockSoulBinder.unlocalisedName, SoulBinderAttractorCystalRecipe.instance);
        MachineRecipeRegistry.instance
                .registerRecipe(ModObject.blockSoulBinder.unlocalisedName, SoulBinderPrecientCystalRecipe.instance);

        // Ender Rail
        if (Config.transceiverEnabled && Config.enderRailEnabled) {
            BasicSoulBinderRecipe err = new BasicSoulBinderRecipe(
                    new ItemStack(Blocks.detector_rail),
                    new ItemStack(EnderIO.blockEnderRail),
                    Config.soulBinderEnderRailRF,
                    Config.soulBinderEnderRailLevels,
                    "EnderRail",
    ...
    public boolean addRecipeFromNBT(NBTTagCompound root) {
        try {
            ...
            if (recipeUid == null || recipeUid.trim().length() == 0) {
                Log.error(
                        "SoulBinderRecipeManager: Could not add custom soul binder recipe from IMC as recipe UID not set: "
                                + root);
                return false;
            }
            ...
            if (inputStack == null) {
                Log.error(
                        "SoulBinderRecipeManager: Could not add custom soul binder recipe from IMC as no input stack defined: "
                                + root);
                return false;
            }
            ...
            if (outputStack == null) {
                Log.error(
                        "SoulBinderRecipeManager: Could not add custom soul binder recipe from IMC as no output stack defined: "
                                + root);
                return false;
            }
            ...
            if (energyRequired <= 0) {
                Log.error(
                        "SoulBinderRecipeManager: Could not add custom soul binder recipe from IMC as energy required was <= 0: "
                                + root);
                return false;
            }
            ...
            if (str == null || str.trim().length() == 0) {
                Log.error(
                        "SoulBinderRecipeManager: Could not add custom soul binder recipe from IMC as no soul types defined: "
                                + root);
                return false;
            }
            ...
            Log.error(
                    "SoulBinderRecipeManager: Could not add custom soul binder exception thrown when parsing message: "
                            + e);
            return false;
        }
    }
