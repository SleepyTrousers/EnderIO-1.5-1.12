...
public class DarkSteelController {
    ...
    private void updateGlide(EntityPlayer player) {
        ...
        if (!player.onGround && player.motionY < 0 && !player.isSneaking() && !player.isInWater()) {
            ...
            double dist = Math.abs(
                    VecmathUtil.distanceFromPointToPlane(plane, new Vector3d(player.posX, player.posY, player.posZ)));
            double minDist = 0.15;
            if (dist < minDist) {
    ...
    private void updateSword(EntityPlayer player) {
        if (ItemDarkSteelSword.isEquipped(player)) {
            ...
            if (Config.darkSteelSwordPowerUsePerHit <= 0
                    || EnergyUpgrade.getEnergyStored(sword) >= Config.darkSteelSwordPowerUsePerHit) {
    ...
    private void doJump(EntityClientPlayerMP player) {
        ...
        if (availablePower > 0 && requiredPower <= availablePower && jumpCount < jumpUpgrade.getLevel()) {
            ...
            player.worldObj.playSound(
                    ...
                    player.posY,
                    player.posZ,
                    EnderIO.MODID + ":ds.jump",
                    1.0f,
                    player.worldObj.rand.nextFloat() * 0.5f + 0.75f,
                    false);
            Random rand = player.worldObj.rand;
            for (int i = rand.nextInt(10) + 5; i >= 0; i--) {
                EntityReddustFX fx = new EntityReddustFX(
                        player.worldObj,
                        player.posX + (rand.nextDouble() * 0.5 - 0.25),
                        player.posY - player.yOffset,
                        player.posZ + (rand.nextDouble() * 0.5 - 0.25),
                        1,
                        1,
                        1);
                fx.setVelocity(
                        player.motionX + (rand.nextDouble() * 0.5 - 0.25),
                        (player.motionY / 2) + (rand.nextDouble() * -0.05),
                        player.motionZ + (rand.nextDouble() * 0.5 - 0.25));
                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            }
            PacketHandler.INSTANCE.sendToServer(
                    new PacketDarkSteelPowerPacket(requiredPower, DarkSteelItems.itemDarkSteelBoots.armorType));
        }
    }
    ...
    public boolean isNightVisionEnch(ItemStack helmet) {
        ...
        if (getNightVisionEnchID() >= 0 && stackEnch != null) {
            for (int i = 0; i < stackEnch.tagCount(); i++) {
                ...
                if (id == getNightVisionEnchID()) return true;
            }
        }
        return false;
    }
