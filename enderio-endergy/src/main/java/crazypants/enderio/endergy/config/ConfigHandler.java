...
public class ConfigHandler {
  ...
  public static void init(FMLPreInitializationEvent event) {
    ...
    if (configDirectory == null) {
      Log.warn(EnderIOEndergy.MOD_NAME + " was initialized before " + EnderIO.MOD_NAME + ". This should not happen.");
      crazypants.enderio.base.config.Config.init(event);
      configDirectory = getConfigDirectory();
    }

    File configFile = new File(configDirectory, EnderIOEndergy.MODID + ".cfg");
  ...
  public static void syncConfig(boolean load) {
    try {
      ...
      Log.error(EnderIOEndergy.MOD_NAME + " has a problem loading its configuration:");
      e.printStackTrace();
    } finally {
  ...
  public static void onConfigChanged(OnConfigChangedEvent event) {
    if (event.getModID().equals(EnderIOEndergy.MODID) || event.getModID().equals(EnderIO.MODID)) {
  ...
  public static void onConfigFileChanged(ConfigFileChangedEvent event) {
    if (event.getModID().equals(EnderIOEndergy.MODID)) {
      Log.info("Updating config...");
      syncConfig(true);
      event.setSuccessful();
