...
public class VecmathUtil {
  ...
  /**
   * Compute the plane equation <code>Ax + By + Cz + D = 0</code> for the plane defined by the three
   * points which lie on the plane, a, b, and c, and placing the result into r. The plane equation
   * can be summarised as the normal vector of the plane (A,B,C) and the distance to the plane from
   * the origin (D).
   * 
   * @param a
   *          vector a.
   ...
   * @return the intersection or null if there is no intersection or the line is on the plane.
   */
  ...
  /**
   * This function computes the ray that goes from the eye, through the specified pixel.
   * 
   * @param camera
   *          the camera.
   * @param x
  ...
  public static void computeRayForPixel(Rectangle vp, Matrix4d ipm, Matrix4d ivm, int x, int y,
    ...
    //Calculate the pixel location in screen clip space (width and height from -1 to 1)
...
//   * <p/>
//   * This routine uses only fast geometric transforms, without costly acos/sin computations. It's
...
//   * <p/>
//   * NB: This is the rotation with shortest angle, which is the one equivalent to the acos/sin
//   * transform method. Other rotations exists, for example to additionally keep a local horizontal
//   * attitude.
//   * 
//   * @param from
...
//
//  /**
//   * Makes a rotation of angle (radians) around the axis specified by the x,y,z values.
//   * 
//   * @param angle
