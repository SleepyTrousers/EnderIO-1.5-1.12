...
@EventBusSubscriber(modid = EnderIOIntegrationForestry.MODID)
public class ConfigHandler {
  ...
  public static void init(FMLPreInitializationEvent event) {
    ...
    if (configDirectory == null) {
      Log.warn(EnderIOIntegrationForestry.MOD_NAME + " was initialized before " + EnderIO.MOD_NAME + ". This should not happen.");
      crazypants.enderio.base.config.Config.init(event);
      configDirectory = getConfigDirectory();
    }

    File configFile = new File(configDirectory, EnderIOIntegrationForestry.MODID + ".cfg");
  ...
  public static void syncConfig(boolean load) {
    try {
      ...
      Log.error(EnderIOIntegrationForestry.MOD_NAME + " has a problem loading its configuration:");
      e.printStackTrace();
    } finally {
  ...
  public static void onConfigChanged(OnConfigChangedEvent event) {
    if (event.getModID().equals(EnderIOIntegrationForestry.MODID) || event.getModID().equals(EnderIO.MODID)) {
  ...
  public static void onConfigFileChanged(ConfigFileChangedEvent event) {
    if (event.getModID().equals(EnderIOIntegrationForestry.MODID)) {
      Log.info("Updating config...");
      syncConfig(true);
      event.setSuccessful();
