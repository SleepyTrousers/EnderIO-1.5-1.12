...
public class ConfigHandler {
  ...
  public static void init(FMLPreInitializationEvent event) {
    ...
    if (configDirectory == null) {
      Log.warn(EnderIOPowerTools.MOD_NAME + " was initialized before " + EnderIO.MOD_NAME + ". This should not happen.");
      crazypants.enderio.base.config.Config.init(event);
      configDirectory = getConfigDirectory();
    }

    File configFile = new File(configDirectory, EnderIOPowerTools.MODID + ".cfg");
  ...
  public static void syncConfig(boolean load) {
    try {
      ...
      Log.error(EnderIOPowerTools.MOD_NAME + " has a problem loading its configuration:");
      e.printStackTrace();
    } finally {
  ...
  public static void onConfigChanged(OnConfigChangedEvent event) {
    if (event.getModID().equals(EnderIOPowerTools.MODID) || event.getModID().equals(EnderIO.MODID)) {
